...
public class BinarySearchTree2_easy {
    ...
    static class BinarySearchTree{
        ...
        // 삭제 과정:
        //
        //리프 노드(자식이 없는 노드) 삭제: 만약 삭제하려는 노드가 리프 노드라면, 그냥 해당 노드를 null로 만들어 삭제합니다.
        //한 쪽 자식만 있는 노드 삭제: 만약 삭제하려는 노드가 한 쪽 자식만 가지고 있다면, 삭제하려는 노드를 건너뛰어 해당 자식을 직접 부모 노드에 연결합니다.
        //왼쪽 자식이 null이면 오른쪽 자식을 반환하여 현재 노드의 위치에 오른쪽 자식을 연결합니다.
        //오른쪽 자식이 null이면 왼쪽 자식을 반환하여 현재 노드의 위치에 왼쪽 자식을 연결합니다.
        //두 자식이 모두 있는 노드 삭제: 삭제하려는 노드가 두 개의 자식을 가지고 있다면, 삭제 노드를 대체할 새로운 값을 찾아야 합니다. 일반적으로 대체 값으로는 삭제 노드의 오른쪽 서브트리에서 가장 작은 값을 가진 노드(오른쪽 서브트리의 최소값) 또는 왼쪽 서브트리에서 가장 큰 값을 가진 노드를 사용합니다. 여기서는 오른쪽 서브트리의 최소값을 사용합니다.
